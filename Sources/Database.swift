...
class DB {
	...
	func create() {
		do {
			...
			defer {
                ...
                try sqlite.execute(statement: "CREATE TABLE IF NOT EXISTS blog (id INTEGER PRIMARY KEY NOT NULL, title TEXT NOT NULL, titlesanitized TEXT NOT NULL, synopsis TEXT NOT NULL, body TEXT NOT NULL, posttime TimeStamp NOT NULL DEFAULT(datetime('now','localtime')),authorid TEXT REFERENCES users(uniqueID) ON DELETE CASCADE, categoryid INTEGER REFERENCES category(id) ON DELETE CASCADE, readtimes INTEGER NOT NULL)")
                try sqlite.execute(statement: "CREATE TABLE IF NOT EXISTS category (id INTEGER PRIMARY KEY NOT NULL, name TEXT NOT NULL)")
	...
	func populate() {
		...
		do {
			...
			for i in 0..<blog_data.count {
				try sqlite.execute(statement: "INSERT INTO blog (id,title,titlesanitized,synopsis,body,authorid,categoryid,readtimes) VALUES(null,:1,:2,:3,:4,:5,:6,:7)", doBindings: {
					(statement: SQLiteStmt) -> () in
					//try statement.bind(position: 1, null)
					try statement.bind(position: 1, blog_data[i][0])
					try statement.bind(position: 2, blog_data[i][0].slugify())
					try statement.bind(position: 3, blog_data[i][1])
					try statement.bind(position: 4, blog_data[i][2])
                    try statement.bind(position: 5, 1)
                    try statement.bind(position: 6, 3)
                    try statement.bind(position: 7, 0)
				})
			}
	...
	func getStory(_ storyid: String) -> [String: String] {
		...
		do {
			...
			let demoStatement = "SELECT blog.title,blog.body,blog.posttime,users.username,category.name FROM blog left join users  on blog.authorid = users.uniqueID join category on blog.categoryid = category.id WHERE titlesanitized = :1"
			...
			try sqlite.forEachRow(statement: demoStatement, doBindings: {
                ...
                data["posttime"] = String(statement.columnText(position: 2))
                data["user_name"] = String(statement.columnText(position: 3))
                data["category_name"] = String(statement.columnText(position: 4))
			})
    ...
    func getCategory() -> [[String: String]]{
        ...
        do{
            ...
            let demoStatement = "SELECT name FROM category"
            try sqlite.forEachRow(statement: demoStatement, handleRow: {
                (statement: SQLiteStmt, i: Int) -> () in
                var contentDict = [String: String]()
                contentDict["name"] = String(statement.columnText(position: 0))
                data.append(contentDict)
            })
        }catch{
            print(error)
    ...
    func setCategory(_ name: String){
        do{
            ...
            let demoStatement = "INSERT INTO category (id,name) VALUES (null,:1)"
            try sqlite.execute(statement: demoStatement, doBindings: {
                (statement: SQLiteStmt) -> () in
